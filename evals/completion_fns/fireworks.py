...
class FireworksBaseCompletionResult(CompletionResult):
...
class FireworksChatCompletionResult(FireworksBaseCompletionResult):
    def get_completions(self) -> list[str]:
        completions = []
...
class FireworksCompletionResult(FireworksBaseCompletionResult):
    def get_completions(self) -> list[str]:
        completions = []
        if self.raw_data and "choices" in self.raw_data:
            for choice in self.raw_data["choices"]:
...
class FireworksCompletionFn(CompletionFn):
    def __init__(
        self,
        model: Optional[str] = None,
        api_base: Optional[str] = "https://api.fireworks.ai/inference/v1",
        api_key: Optional[str] = os.environ.get("FIREWORKS_API_KEY"),
    ...
    def __call__(
        ...
        **kwargs,
        ...
        result = FireworksCompletionResult(raw_data=result, prompt=openai_create_prompt)
        record_sampling(prompt=result.prompt, sampled=result.get_completions())
        return result


class FireworksChatCompletionFn(CompletionFnSpec):
    def __init__(
        self,
        model: Optional[str] = None,
        api_base: Optional[str] = "https://api.fireworks.ai/inference/v1",
    ...
    def __call__(
        ...
        **kwargs,
        ...
        result = FireworksChatCompletionResult(
